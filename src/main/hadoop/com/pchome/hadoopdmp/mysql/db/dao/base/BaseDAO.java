package com.pchome.hadoopdmp.mysql.db.dao.base;
...
public abstract class BaseDAO<T, PK extends Serializable> extends HibernateDaoSupport implements IBaseDAO<T, PK> {
	...
	public List<T> loadAll(int firstResult, int maxResults) {
		return getHibernateTemplate().getSessionFactory().getCurrentSession().createCriteria(getMyClass())
	...
	protected int nextVal(final String sql) {
		BigInteger nextval = getHibernateTemplate().execute(new HibernateCallback<BigInteger>() {
