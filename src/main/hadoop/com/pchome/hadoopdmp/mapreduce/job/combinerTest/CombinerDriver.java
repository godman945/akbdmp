...
public class CombinerDriver {
	...
	private static Log log = LogFactory.getLog("CombinerDriver");
	...
	public void drive(String env,String timeType) throws Exception {
		try {
			...
			Job job = new Job(jobConf, "combiner_log_"+ env + "_" + sdf.format(date));
...
//		System.out.println(timePath);
		...
		CombinerDriver combinerDriver = (CombinerDriver) ctx.getBean(CombinerDriver.class);
		combinerDriver.drive(args[0],args[1]);
		log.info("====driver end====");
	}
//
}
